        ...
        <table class="min-w-full border border-gray-300 text-black rounded-lg shadow-sm overflow-hidden">
            <thead class="bg:  #444A68">
                <tr>
                    ...
                    <th class="border border-gray-300 px-4 py-2 text-left">Publsiher</th>
            ...
            <tbody>
                ...
                <tr class="hover:bg-gray-50">
                    <td class="border border-gray-300 px-4 py-2 rounded-l-lg">{{ $book->isbn }}</td>
                    <td class="border border-gray-300 px-4 py-2 rounded-l-lg">{{ $book->name }}</td>
                    <td class="border border-gray-300 px-4 py-2 rounded-l-lg">{{ $book->publisher->name ?? '-' }}</td>
                    <td class="border border-gray-300 px-4 py-2 rounded-l-lg">
                        @foreach ($book->authors as $author)
                        {{ $author->name }}@if(!$loop->last), @endif
                        @endforeach
                    </td>
                    <td class="border border-gray-300 px-4 py-2 rounded-l-lg">{{ $book->bibliography }}</td>
                    <td class="border border-gray-300 px-4 py-2 rounded-l-lg">
                        @if ($book->cover_image)
                    ...
                    <td class="border border-gray-300 px-4 py-2 rounded-l-lg">€{{ number_format($book->price, 2) }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
