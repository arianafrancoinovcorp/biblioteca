...
class RequestsController extends Controller
{
    ...
    public function index()
    {
        ...
        $deliveredTodayCount = BookRequests::whereDate('return_date', Carbon::today())->count();

        return view('requests.index', compact('requests', 'activeRequestsCount', 'last30DaysCount', 'deliveredTodayCount'));
    }
    ...
    public function store(Request $request)
{
    ...
    public function show($id)
    {
        ...
        if (!Auth::user()->isAdmin() && $bookRequest->user_id !== Auth::id()) {
            abort(403, 'Access denied');
        }

        return view('requests.show', ['request' => $bookRequest]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $bookRequest->return_date = now()->toDateString();
        $bookRequest->status = 'delivered';
        $bookRequest->save();

        return redirect()->back()->with('success', 'Book return confirmed');
    }

    public function returnBook(BookRequests $request)
    {
